...
### 新增补充（近期变更）
- 播放器删除广播的稳定性：在 `src/renderer/src/pages/modules/songPlayer/usePlayerControlsLogic.ts` 的 `delSong()` 内，使用删除开始时的 `currentSongListUUID` 快照作为 `songsRemoved`/`playlistContentChanged` 的 `listUUID`，避免在清空/切歌后 UUID 变动导致的错位。
- 全局播放快照同步：在 `src/renderer/src/App.vue` 的 `onMounted` 中监听 `songsRemoved`，若命中当前播放歌单，则同步过滤 `runtime.playingData.playingSongListData`，并在需要时将 `playingSong` 置空，杜绝旧快照“复活”。
- 防御性去重：在 `src/renderer/src/pages/modules/songsArea/composables/useSongsAreaColumns.ts` 的 `applyFiltersAndSorting()` 尾部按 `filePath` 去重，抵御极端竞态引入的重复条目。
- 快捷键语义明确：保留 F 用于删除“当前正在播放”的歌曲；Delete 由列表区删除选中项。两者不再混用，避免语义歧义引发竞态。
...
新增建议
- 若再次遇到 F 删除后“复活”，核对播放器删除链路中 `listUUID` 是否来自删除起点的快照；同时确认 `App.vue` 的全局监听已对 `playingSongListData` 做到同步过滤。
...
补充期望（针对播放器删除/F 键场景）
- `EVENT_songsRemoved` 的 `listUUID` 应等于删除开始时的播放歌单 UUID（不会为空或被后续状态覆盖）。
- F 删除时应仅出现一次 `EVENT_songsRemoved removeCount: 1`，随后 `APPLY_START/DONE` 的 `originalLen/runtimeLen` 均递减 1，不会出现已移动曲目重新出现在当前歌单。
