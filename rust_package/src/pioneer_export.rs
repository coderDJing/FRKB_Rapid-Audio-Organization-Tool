...
fn collect_raw_playlist_entries(pdb_path: &Path, playlist_id: u32) -> Vec<RawPlaylistEntry> {
  ...
  loop {
    ...
    let mut record_offset = data_start;
    while record_offset.saturating_add(12) <= data_end {
...
fn recover_complete_raw_playlist_entries(
  ...
  for &(entry_index, track_id) in playlist_entries.iter() {
    ...
    match indexed_entries.insert(entry_index, track_id) {
      Some(existing_track_id) if existing_track_id != track_id => return,
      _ => {}
    }
  }
  let Some(&last_entry_index) = indexed_entries.last_key_value().map(|(index, _)| index) else {
    return;
  };

  let mut raw_entries_by_index = BTreeMap::new();
  for raw_entry in raw_entries {
    if raw_entry.entry_index > last_entry_index || !track_map.contains_key(&raw_entry.track_id) {
      continue;
    }
    match raw_entries_by_index.insert(raw_entry.entry_index, raw_entry.track_id) {
      Some(existing_track_id) if existing_track_id != raw_entry.track_id => return,
      _ => {}
    }
  }
  ...
  for entry_index in 1..=last_entry_index {
    if indexed_entries.contains_key(&entry_index) {
      continue;
    }
    let Some(&track_id) = raw_entries_by_index.get(&entry_index) else {
      return;
    };
    recovered_entries.push((entry_index, track_id));
  }
  if recovered_entries.is_empty() {
    return;
  }

  playlist_entries.extend(recovered_entries);
...
pub fn read_pioneer_playlist_tracks(
  ...
  let raw_playlist_entries = collect_raw_playlist_entries(pdb_path, playlist_id);
  recover_complete_raw_playlist_entries(&mut playlist_entries, &raw_playlist_entries, &track_map);
  playlist_entries.sort_by(|left, right| left.0.cmp(&right.0));
