        ...
        remove_result = _run_bridge(
            ...
            "remove-playlist-tracks",
            {
                **_bridge_payload(db_path),
                ...
                "rowKeys": [str(item.get("sourceRowKey") or "") for item in differences],
            },
        )
...
def _load_report_for_apply(report_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    return summary, [item for item in differences if isinstance(item, dict)]
...
def _apply_existing_report(
    *,
    ...
    return _apply_playlist_updates(
        bridge_path,
        db_path,
...
def main() -> int:
    ...
    if not dry_run:
        apply_result = _apply_playlist_updates(
            bridge_path,
            str(args.db_path or ""),
