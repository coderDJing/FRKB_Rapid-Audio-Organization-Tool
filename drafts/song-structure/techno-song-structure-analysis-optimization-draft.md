...
> 当前状态（2026-07-20）：冻结基线为原生四拍网格 `v27`，当前待试听候选为 `v32`。v26 起不再依赖大节线、
...
> v28 增加局部边界所有权精修，区分段落变化提示块与新状态真正成立的第一块。
> v29 增加 Active / Inactive 宏观状态约束，以及保留低频基础的终段累计退出识别。
> v30 限制 Build 拓扑扩张跨越已经恢复低频基础的 Active 平台，避免把多个 Drop / Breakdown 压成一段。
> v31 将 Outro 的持续性确认窗口与实际起点定位分离，避免后续坍缩把边界预判到数个四拍块之前。
> v32 增加方差感知的多变量左右窗口变化证据，只用于已有结构候选的落点择优；同时以软 motif
> 相似度补充硬 cluster 重复统计，不允许多变量证据脱离谱聚类和方向事件独立制造边界。
> 本文保留架构
> 演进和历史实验背景；
...
状态：历史架构与实验记录；冻结基线为 v27，当前待试听候选为 v32，真实样本与验收口径见唯一流程文档
...
- v28 在最终语义稳定后增加局部边界所有权精修：只在 Build 前候选块是孤立抽空，或 Drop 前候选块
  只有高频冲击、下一块才出现持续低频基础落地时，把边界向后对齐一个四拍块。该规则使用本曲局部
  新状态原型和持续性证据，不做全局时间偏移，并保护已经立即成立的 Build / Drop 起点。
- v29 先按本曲 activity / foundation 分布建立 Active / Inactive 宏观平台，避免仅凭相对 Drop 降能和
  状态转移模板把仍有完整节奏基础的低能 Active 平台标成 Breakdown；确认低活动平台后，只允许把
  “低频仍在、上层先抽离”的持续拆层块向前纳入 Breakdown。终段另增加 4 / 8 / 12 个四拍块的累计
  退出证据，仅识别基础仍在但上层编排持续退出且不恢复的 Outro，避免吞并普通末段 Groove。
- v29 的 landing / switch 候选使用有上限的相对平局容差，避免本曲归一化后的极小阈值差把真实四拍
  落点删掉；直接 Build 的回溯不能跨越已经完成的 Breakdown，防止为了延长 Build 反吃前一段状态。
- 首段不再只靠固定时长结束 Intro：首次明确节奏基础落地、且局部 Groove 证据胜出时允许在该四拍点
  切入 Groove；反过来，Build 前若短段的局部 Groove 明显胜出，也禁止状态拓扑模板把它伪装成
  Breakdown。两条规则都依赖本曲局部原型和持续性，不绑定曲名、绝对时间或固定第几个段落。
- 2026-07-17 本机 9 首真实音频回归全部成功、无解码错误；其中 5 首具有可计分 approved truth，
  `boundary F1 = 0.918033`、`strict label accuracy = 0.964722`。新增样本完整 9 段边界与标签均为 100%，
  但它已经参与 v29 调参，只能作为 development 回归证据，不能据此宣称 blind holdout 泛化提升。
- v30 将上下文 Build 的职责限制在 Breakdown 内部或其后的未决状态，不允许向前覆盖已经成立的
  Active 平台；长 Build 向后续 landing 扩张时，若中间出现至少 8 个四拍块、且低频基础已经接近
  landing 平台的完整 Active 重入，则把它视为结构屏障。只有高频活跃、低频基础仍未落地的假 Drop
  不触发该屏障，因此保留真实的长 Build。
- v31 继续使用 4 / 8 / 12 个四拍块确认末段退出是否持续，但不再直接把四块确认窗口的最左端作为
  Outro 边界。确认成立后，在窗口内部逐块寻找承担主要下降强度、且下一块没有恢复的真实 onset；
  单块短降后恢复仍归前一 Drop，避免把后面才发生的主坍缩提前两到三个四拍块。
- v31 完成本机 9 首真实音频回归后仍为 9 / 9 成功、5 首 approved truth 可计分，聚合指标保持
  `boundary F1 = 0.918033`、`strict label accuracy = 0.964722`；该结果仍属于已消费开发集回归，
  不等同于新的 holdout 泛化证明。
- v32 参考判别式变化点与软重复分解思路，在当前四拍特征上比较候选边界左右窗口的多变量均值、
  窗内离散程度、local vector 与 recurrence vector。首轮实验允许该分数独立生成 Switch 时造成明显
  过分段，因此正式候选只把它作为既有谱聚类边界附近的落点择优证据；单个四拍毛刺不得触发。
- v32 的重复性同时考虑硬 cluster 出现次数与远端段落的软 motif 相似度。软相似度要求持续段级
  向量接近、时长相容并降低相邻切片权重，主要用于识别“核心编曲相同但叠加层略有变化”的重复段。
  本机 9 首回归仍为 9 / 9 成功，5 首 approved truth 指标保持 `boundary F1 = 0.918033`、
  `strict label accuracy = 0.964722`；review-queue 中仅有一首 Outro 起点提前一个四拍块，需要试听确认，
  不得宣称准确率提升。
