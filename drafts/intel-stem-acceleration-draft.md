...
- 现有代码对 `xpu` 和 `directml` 只做了 PyTorch 路线支持。
...
这一节是给后续真实开工排期用的，不再只是原则描述，而是建议的改造节奏。

### 第一阶段：冻结术语与平台策略

先把平台规则、术语和优先级写死，再开始动实现。

目标：

- 冻结 `CUDA / XPU / NPU / DirectML / MPS / CPU` 定义
- 冻结 `macOS: MPS > CPU`
- 冻结 `Windows: CUDA > XPU > NPU > DirectML > CPU`
- 冻结 `Windows AMD = DirectML`
- 冻结 `老 Mac AMD = MPS`

交付标准：

- 文档与代码命名一致
- `resolveStemDevicePriority()` 改造目标明确
- 后续实现不会再因为术语打架返工

### 第二阶段：先落地 Windows `XPU`
...
- 安装包能带 `runtime-xpu`
- Intel Arc 机器可被稳定探测到 `xpu`
- `xpu` 排在 `directml` 前
- 失败时能稳定回退到下一个后端

交付标准：

- `scripts/demucs-runtime-profiles.json` 中 `xpu` 不再是空壳
- `prepare/ensure` 两个脚本都能正确处理 `runtime-xpu`
- 本机或真机测试上 `xpu` 可完成完整 stem 任务

### 第三阶段：重构执行链和输入链

在 `xpu` 能被正式选中之后，尽快把执行层从黑盒 CLI 里拽出来。

目标：

- 统一 bootstrap 入口
- 解决 `torch.load(weights_only)` 兼容问题
- 摆脱 `torchaudio/torchcodec` 的脆弱依赖
- 使用更稳定的 waveform-first 执行链

交付标准：

- `cuda/xpu/directml` 共用同一执行主干
- 音频解码不再依赖 `torchaudio.load`
- 输出写盘链稳定

### 第四阶段：补 benchmark cache 与平台化自动调度
...
NPU 放最后，不是因为它没价值，而是因为它不能污染主线稳定性。
...
- NPU 路线可单独启停
- 即便失败也不影响 `CUDA / XPU / DirectML / CPU`
- 研究结果可以独立评估，不绑死主线发布时间
