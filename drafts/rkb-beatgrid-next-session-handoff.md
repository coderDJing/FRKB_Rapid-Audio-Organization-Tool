...
`constant-grid-dp` 验收链路已接入 locked rising-edge ranker、保守的 legacy integer BPM snap、rank1 material legacy weakness switch、rank1 structural phase switch、rank1 high structural score switch、head near-zero switch、rank1 negative legacy score v2 switch，以及 rank1 octave-down switch。八者都走同一套 solver 入口，已覆盖 Electron 实时分析路径。

当前 `scripts/rkb_constant_grid_dp_solver.py` 版本：

```text
constant-grid-dp-cache-v3-locked-rising-edge-ranker-integer-bpm-snap-rank1-material-legacy-weakness-v3-rank1-structural-phase-v2-rank1-high-structural-score-v1-rank1-negative-legacy-score-v2-head-near-zero-v1-rank1-octave-down-v1
```
...
`D:/FRKB_database-E/library/FilterLibrary/sealed-eval`。本轮
...
- audio：`D:/FRKB_database-E/library/FilterLibrary/sealed-eval`
...
下一次泛化证明必须来自另一批全新 Rekordbox playlist。
...
- audio：复跑时使用 `D:/FRKB_database-E/library/FilterLibrary/sealed-eval;D:/FRKB_database-E/library/FilterLibrary/sample;D:/FRKB_database-E/library/FilterLibrary/grid-failures-current`
...
rank1 high structural score v1、rank1 negative legacy score v2、rank1 octave-down 的泛化，需要另一批全新 Rekordbox playlist
原样 sealed 复验。
...
1. 当前 production solver 是否能原样泛化：
   `locked rising-edge ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2 + rank1 high structural score v1 + rank1 negative legacy score v2 + head near-zero + rank1 octave-down`
   必须不改阈值、不改 guard、不重训，直接跑 sealed-eval。
2. `rank1 structural phase v2` 的低概率高证据分支、`rank1 high structural score v1`、`rank1 negative legacy score v2`
   ...
   `0.85` / `0.8` / `0.0`。
   如果 rank1 high structural score 触发，要单独记录 `probability`、`score`、
   `downbeatMargin`、`phasePathScore`、`tempoScore`、`legacyFirstBeatMs`、相位差、BPM 差、
   `leadingEdgePeakOffsetMadMs` 和 `downbeatRank`；不能根据触发曲目现场改
   `0.82` / `0.96` / `20ms`。
...
integer BPM snap、rank1 material legacy weakness v3、rank1 structural phase v2、rank1 high structural score v1、
rank1 negative legacy score v2、head near-zero 和 rank1 octave-down；这一步就是主验收输出，不要在 sealed-eval 上现场改阈值：
...
- current consumed regression 音频状态：`sealed-eval = 1412`，`sealed-intake = 316`；
  `sealed-intake` 当前仍不可当 fresh sealed 入口，下一批前必须归档/清空。
