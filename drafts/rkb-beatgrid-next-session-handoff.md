...
`constant-grid-dp` 验收链路已接入 locked rising-edge ranker、保守的 legacy integer BPM snap、rank1 material legacy weakness switch、rank1 structural phase switch、head near-zero switch、rank1 negative legacy score v2 switch，以及 rank1 octave-down switch。七者都走同一套 solver 入口，已覆盖 Electron 实时分析路径。

当前 `scripts/rkb_constant_grid_dp_solver.py` 版本：

```text
constant-grid-dp-cache-v3-locked-rising-edge-ranker-integer-bpm-snap-rank1-material-legacy-weakness-v3-rank1-structural-phase-v2-rank1-negative-legacy-score-v2-head-near-zero-v1-rank1-octave-down-v1
```
...
- 打包资源：`package.json` 与 `electron-builder.yml` 已补充 `beat_this_runtime_constant_grid.py`、`benchmark_rkb_rekordbox_truth.py`、`rkb_*.py` 到 `demucs/bootstrap`，并覆盖
  `rkb_constant_grid_dp_cache.py`、`rkb_constant_grid_dp_cli.py`、`rkb_constant_grid_dp_octave.py`
  这些 solver 拆分模块。
...
- current production latest：`707 / 931 = 75.94%`，error `0`；
...
- current 音频目标分布：`new = 0`，`sample = 707`，
  `grid-failures-current = 224`。
...
rank1 negative legacy score v1/v2 开发回归，后续只能当普通回归集。
...
- final selected：`224 / 353 = 63.46%`
- errorTrackCount：`0`
- category：`pass 224`，`first-beat-phase 84`，`downbeat 15`，`bpm 17`，
...
```text
rising-edge locked ranker + legacy integer BPM snap + rank1 material legacy weakness + rank1 structural phase v2 + rank1 negative legacy score v2 + head near-zero + rank1 octave-down
```
...
rank1 negative legacy score v2、rank1 octave-down 的泛化，需要另一批全新 Rekordbox playlist
原样 sealed 复验。
...
latest test353 和 latest test316 都已经被
看过并参与过开发判断，继续用它们涨分只算回归，不算泛化。

下一批新样本只验证这些事：

1. 当前 production solver 是否能原样泛化：
   `locked rising-edge ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2 + rank1 negative legacy score v2 + head near-zero + rank1 octave-down`
   必须不改阈值、不改 guard、不重训，直接跑 sealed-eval。
2. `rank1 structural phase v2` 的低概率高证据分支、`rank1 negative legacy score v2`
   和 `rank1 octave-down` 是否在 fresh 样本上仍是净正向：
   ...
   `0.85` / `0.8` / `0.0`。
   如果 rank1 octave-down 触发，要单独记录 `confidence`、`rank1Bpm`、`legacyBpm`、
   `score`、`downbeatMargin`、`phasePathScore`、`leadingEdgePeakOffsetMadMs`
   和 `tempoScore`；不能根据触发曲目现场改 `0.82` / `0.86` / `0.74`。
...
integer BPM snap、rank1 material legacy weakness v3、rank1 structural phase v2、rank1 negative legacy score v2、
head near-zero 和 rank1 octave-down；这一步就是主验收输出，不要在 sealed-eval 上现场改阈值：
...
- `py_compile` 通过：`rkb_constant_grid_dp_selection.py`、`rkb_constant_grid_dp_solver.py`、
  `rkb_constant_grid_dp_octave.py`、`rkb_constant_grid_dp_cache.py`、`rkb_constant_grid_dp_cli.py`。
- `npx vue-tsc --noEmit` 通过。
- `git diff --check` 通过（仅 Git CRLF 提示）。
- 单曲 smoke 通过：
  - `Umoja - Rabia (Original Mix).mp3`：`half-or-double-bpm -> pass`，guard `rank1-octave-down`
  - `Club des Belugas - It Don't Mean a Thing.mp3`：`half-or-double-bpm -> pass`，guard `rank1-octave-down`
...
- rank1 negative legacy score v2 回归：current `706 / 931`，blind `436 / 608`，
  latest test353 `224 / 353`；current/blind/test353 逐曲 diff 均 `pass -> fail = 0`。
- rank1 octave-down 回归：current `707 / 931`，blind `436 / 608`，latest test327
  `218 / 327`，latest test316 `176 / 316`；current/blind/test327/test316 逐曲 diff 均
  `pass -> fail = 0`。
- current classification / sample-regression / grid-failures-current 派生视图已刷新到 `707 / 931`。
- E 音乐库 current 目标分布为 `new = 0`、`sample = 707`、`grid-failures-current = 224`，
  同步后 `sync_frkb_classification_audio_dirs.py --dry-run` 必须为 `moveCount = 0`。
- 本轮实际同步移动 `Umoja - Rabia (Original Mix).mp3`：
...
- 临时 `*.progress.json` 与 `*.shards` 已按用户确认清理完毕；当前剩余数量为 0。
