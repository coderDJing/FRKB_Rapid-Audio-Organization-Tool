...
> [`准备好rkb新样本.md`](./准备好rkb新样本.md) 为准。当前已有冻结 v3，所以下一批完整 review roster
> 优先走 `--fresh-validation`；只有没有待验证冻结候选时才直接进入 reviewed development。

## 1. 当前口径

当前 usable-grid 主口径：

- `0.5x / 1x / 2x` tempo family 都允许；先把候选 BPM 归一到 truth tempo；
- 归一化后 `firstBeatPhaseAbsErrorMs <= 5ms`；
- 归一化后 `gridMaxAbsMs <= 5ms`；
- 归一化后 `bpmOnlyDrift128BeatsMs <= 5ms`；
- exact BPM 是否与 Rekordbox 一致只作 diagnostic；非 octave tempo 不允许；
- downbeat 单列验收，不能用 phase/tempo 的净增把新增 downbeat 错误藏掉。
...
fixed/no-fit nested LOBO runner 已实现并完成首个六折 study，但该 study 六折均选择 baseline、净增 0、
aggregate gate 未通过。后续 v2 的 BPM gate 已被产品口径纠正；corrected-label v3 为 6/6 折正向并通过
development gates，但仍是 consumed post-hoc candidate，只能等待下一批 fresh，不能误写成 production 提升。
...
- exact BPM drift diagnostic 与非 octave tempo failure。
- phase 误差分布。
- downbeat 回归。
- 是否使用了任何禁止字段。
...
- consumed 全量 usable-grid 正确率相对同版本 baseline 有预注册净增；晋级前必须验证多数 LOBO 同向。
- runner 产出的 LOBO 最差批次、非 octave tempo、downbeat 净增和新增 downbeat 数不触发门槛。
...
统一入口当前 v2 acceptance policy：usable-grid 净 pass `>= 1`、error rate `= 0`、downbeat failure
净增 `<= 0.5%`、新增 downbeat failure `<= 0.5%`、非 octave tempo failure `= 0`、usable candidate
oracle `>= 94%`。500 首时两个 downbeat 上限都最多 2 首。门槛写入 immutable manifest 后不能修改；
只有自动通过锁死 policy 的结果才能 finalize 为 `eligible`，且 `eligible` 不直接执行 production promotion。
...
统一 solver baseline，并复用已经实现的 nested LOBO runner 对新候选开独立 study，再锁死通过门槛的候选
交给下一批 fresh 原样复验。需要训练的新 scorer 只能扩展现有 runner 的 fold 内 trainer contract，
禁止另起一套缺少 exposure ledger 的评估流程。
registry 与 stable-assignment split 四件套已完成；new357 剩余 355 首强身份重算前不算可靠全量回放。

多尺度 spectral flux 的 2026-07-16 development study 证明三尺度频谱证据能提供小幅信号，但首次 v2
报告使用了错误的产品门槛：半倍/双倍 BPM 被当成失败，且 `bpmBigError` 实际只是 128 拍累计漂移超过
`5ms`，不能解释成三倍、四倍 BPM 灾难。项目现允许 `0.5x / 1x / 2x`，前提是归一化后的 BPM、phase、
grid max 都在 `5ms` 内；精确 BPM 只作诊断，downbeat 单列 gate。

冻结 v2 replay 没有重训或重选 threshold，并完整复现旧 strict `+5 / 3388`。唯一 `100 -> 200 BPM`
退化在归一化后 phase/grid 均为 `5ms`，确实应算 `octave-equivalent-pass`。但 replay 还发现 split truth
字段是 `barBeatOffset`，旧 development adapter 却读取 `downbeatBeatOffset`，3745 首中 536 首非零值曾被
静默按 0 评估。纠正 schema 后，v2 为 `3 / 1 / 2` 个正/中/负 fold，macro usable `+0.202264%`、
净 usable pass `+5`、usable `pass -> fail = 4`，最差 fold `-0.611621%`、最差 pass-to-fail
`0.917431%`、最差 downbeat 增量 `0.611621%`，仍未通过。失败根因现在是跨折稳定性和真实
phase/downbeat 退化，不是可接受的倍频 BPM。禁止接 production，也禁止继续只扫 threshold。

按 corrected usable-grid labels 重训的 post-hoc v3 随后得到 6/6 折正向，macro `+1.0395685%`、
净 usable pass `+30 / 3388`、usable `pass -> fail = 0`、最差 fold `+0.3058104%`，最差 downbeat
failure 净增 `+0.3289474%`，development aggregate 通过。完整性检查覆盖 corrected row
`3388 / 3388`、54208 个候选、fold overlap `0` 和禁止字段 feature leak `0`。最终候选已按六折
inner-selected exact config 众数冻结为 `multiscale / l2=1 / top16 / threshold=1.1`，SHA-256 为
`28e92006d712a024f4488ddfab5b2a5e5dec12de7a1cb6075402ea21cc9c6207`。

这仍不是 fresh 证明：v3 是看过 v2 后形成的 consumed development candidate，且 43 次 category-change
选择了半速候选；这符合已明确的产品 tempo family。另有 5 次单曲 downbeat 新增失败，只是被其他
downbeat 改善抵消后通过 development net gate。fresh 已锁为双门槛：净增和新增错误率都 `<= 0.5%`，
500 首各最多 2 首；冻结后禁止继续改模型或拿旧 outer 调 threshold，当前也禁止直接接 production。
...
3. fixed/no-fit nested runner 已实现；训练型 scorer 复用现有 fold plan 做 development adapter。新 study
   必须先把 `barBeatOffset` / `downbeatBeatOffset` 规范化，并以 usable-grid label 为主指标；精确 BPM
   漂移只作诊断，非 octave tempo、phase 和 downbeat 回归继续单列，最终仍要等待 fresh。
