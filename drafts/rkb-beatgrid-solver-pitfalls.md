...
当前 production latest（v3 + rank1 structural phase v2 + rank1 high structural score v1 + rank1 negative legacy score v2 + head near-zero + rank1 octave-down）：
...
- current all 高于当前 production latest `976 / 1407 = 69.37%`。
- current holdout 高于当前 production `176 / 288 = 61.11%`。
...
当前 production latest 以 v3 + rank1 structural phase v2 + rank1 high structural score v1、
rank1 negative legacy score v2、head near-zero + rank1 octave-down 为准：current
`976 / 1407 = 69.37%`，current holdout `176 / 288 = 61.11%`，
