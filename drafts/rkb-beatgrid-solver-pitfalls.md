...
当前 production selected（v3 + rank1 structural phase v2 + head near-zero + rank1 negative legacy score）：

| dataset | selected | candidate oracle | 主要剩余失败 |
| --- | ---: | ---: | --- |
| current all | `706 / 931 = 75.83%` | `899 / 931 = 96.56%` | `first-beat-phase 176`，`downbeat 32`，`bpm 16`，`half/double 1` |
| blind all | `436 / 608 = 71.71%` | `599 / 608 = 98.52%` | `first-beat-phase 126`，`downbeat 30`，`bpm 15`，`half/double 1` |
| latest test353 | `224 / 353 = 63.46%` | `338 / 353 = 95.75%` | `first-beat-phase 82`，`downbeat 17`，`bpm 17`，`half/double 12`，`grid-drift 1` |
| consumed test327 | `215 / 327 = 65.75%` | `316 / 327 = 96.64%` | `first-beat-phase 65`，`downbeat 26`，`bpm 8`，`half/double 13` |
...
当前主 truth 有 `193` 首、blind 有 `163` 首、latest test353 有 `114` 首是候选池里存在 5ms passing candidate，但最终 scorer
...
locked rising-edge ranker 已集成到 `constant-grid-dp`，后续又加了五条窄 guard：
...
- `constant-grid-dp-rank1-negative-legacy-score-switch`：只在前序 switch 都未切换、baseline
  ...
  bar offset mod4 相同、`downbeatRank == 0` 时生效。它只看 rank1，不扫 topN，不读歌名、
  artist/title/path、truth、category、pass/fail 或 split identity。结果为 current
  `702 -> 706`，blind `436 -> 436`，latest test353 `222 -> 224`；current/blind/test353
...
这五条都不能包装成新的 sealed 泛化证据：`test` 已被继续优化消耗，current / blind 也是已看过的
...
- current all 高于当前 production `706 / 931 = 75.83%`。
- current holdout 高于当前 production `138 / 203 = 67.98%`。
- blind all 不低于当前 production `436 / 608 = 71.71%`。
...
当前 production baseline 以 v3 + rank1 structural phase v2 + head near-zero + rank1 negative legacy score
为准：current `706 / 931 = 75.83%`，current holdout `138 / 203 = 67.98%`，
blind `436 / 608 = 71.71%`。
...
- integrated solver 已接入 locked ranker、legacy integer BPM snap、rank1 material legacy weakness、
  rank1 structural phase、head near-zero 和 rank1 negative legacy score：
  current `685 -> 694 -> 695 -> 696 -> 701 -> 702 -> 702 -> 706`，blind
  `425 -> 430 -> 430 -> 432 -> 434 -> 435 -> 436 -> 436`，old consumed test
  `274 -> 277 -> 278 -> 279`，latest test-new-357 `229 -> 230 -> 231`，
  latest test353 `222 -> 224`。三套逐曲均无 `pass -> fail`。其中 old consumed test
  只有 locked ranker 阶段仍可算 fresh sealed 证据；后续 guard、latest test-new-357 的 v3
  边界、structural phase v2、test327 的 head near-zero 和 test353 的 rank1 negative legacy score
  都只是开发回归证据。
