...
当前 production latest（v3 + rank1 structural phase v2 + rank1 negative legacy score v2 + head near-zero + rank1 octave-down）：

| dataset | selected | candidate oracle | 主要剩余失败 |
| --- | ---: | ---: | --- |
| current all | `707 / 931 = 75.94%` | `899 / 931 = 96.56%` | `first-beat-phase 176`，`downbeat 32`，`bpm 16` |
| blind all | `436 / 608 = 71.71%` | `599 / 608 = 98.52%` | `first-beat-phase 126`，`downbeat 30`，`bpm 15`，`half/double 1` |
| latest test353 | `224 / 353 = 63.46%` | `338 / 353 = 95.75%` | `first-beat-phase 84`，`downbeat 15`，`bpm 17`，`half/double 12`，`grid-drift 1` |
...
当前主 truth 有 `192` 首、blind 有 `163` 首、latest test353 有 `114` 首是候选池里存在 5ms passing candidate，但最终 scorer
...
- current all 高于当前 production latest `707 / 931 = 75.94%`。
...
当前 production latest 以 v3 + rank1 structural phase v2 + rank1 negative legacy score v2、
head near-zero + rank1 octave-down 为准：current `707 / 931 = 75.94%`，
current holdout `138 / 203 = 67.98%`，
blind `436 / 608 = 71.71%`。
...
- integrated solver 已接入 locked ranker、legacy integer BPM snap、rank1 material legacy weakness、
  rank1 structural phase、rank1 negative legacy score v2、head near-zero 和 rank1 octave-down：
  ...
  latest test327 `212 -> 215 -> 218`，latest test353 `222 -> 224 -> 224`，
  latest test316 `175 -> 176`。三套逐曲均无 `pass -> fail`。其中 old consumed test
