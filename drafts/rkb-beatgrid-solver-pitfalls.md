...
后续不要把小模型当主线；它只能用来验证新 phase 特征有没有信号，不能替代 phase 语义本身。
...
locked rising-edge ranker 已集成到 `constant-grid-dp`，后续又加了三条窄 guard：
...
这三条都不能包装成新的 sealed 泛化证据：`test` 已被继续优化消耗，current / blind 也是已看过的
...
当前 5ms `frkb-current-latest.json` replay 结论：

- 现有 selected：`685 / 931 = 73.58%`，holdout `130 / 203 = 64.04%`。
