...
这不等于实现上没有冲突：FRKB 的树节点 UUID、相对路径、缓存键、Set/Mixtape 关联记录都
...
导入、文件操作或歌单菜单中。点击后打开文件选择器，筛选并只接受 `FRKB.database.frkbdb`。
...
- 来源 manifest 的最低应用版本高于当前 FRKB。
- 来源 SQLite 不存在、无法只读打开、`PRAGMA integrity_check` / 外键检查失败，或 schema
  版本不在首版支持范围内。
...
复制按实际字节数；数据库提交和来源删除显示各自明确阶段。这样进度不会因某个大文件或缓存
blob 而长期停在一个误导性的百分比上。
...
- 已迁移的可保留分析数据数，以及因版本不匹配而安排重新分析的缓存数。
...
| 可验证的曲目分析 | `song_cache` 的 `info_json`（含 key、BPM、网格、cue、能量、段落等）、封面索引、波形/显示波形缓存。按映射重写键与 `info_json.filePath`，只在目标文件实际 size/mtime 可匹配时写入。 |
| 可丢弃缓存 | 与外接 Rekordbox/USB 设备绑定的 `external_analysis_*`，以及任何没有注册重写规则的派生缓存。它们不从来源复制，后续由当前库重新建立。 |

复制时应请求保留文件时间戳；复制结束后仍以目标实际 `stat` 写回缓存中的 `size` / `mtime_ms`。
若目标文件系统不能精确保留时间戳，相关缓存必须显式失效并在结果页计数，不能保留一个永远不会命中的
缓存条目。

`meta` 属于目标库自身状态，不从来源覆盖。来源 manifest、来源 SQLite 文件、WAL/SHM 文件和旧版
`songFingerprint` 文件均不直接复制；必须从已验证的数据读取后重建目标状态。

为了防止后续 schema 新增表时漏搬数据，合并实现应维护“表处理注册表”：每张表必须显式标记为
`transform`、`union`、`discardable` 或 `unsupported`。发现未知业务表时默认失败；不能默认跳过。
...
| 内容 | 合并策略 |
| --- | --- |
| Filter / Curated 普通歌单 | 复制歌单文件，重建节点 UUID，迁移可验证分析缓存与指纹。 |
...
保守增量上界，`J` 为任务日志/路径映射上界，`H` 为安全余量，则各阶段的目标卷新增占用为：

| 阶段 | 新增占用上界 | 说明 |
| --- | --- | --- |
| 完整预检 | 近似 `J` | 计划先在内存生成；持久任务日志尚未创建或只有极小元数据。 |
| 暂存复制 | `P + J` | 每个来源文件只写一个目标 `.part` / 暂存文件；流式 hash 不会再复制一份。 |
| 同卷提升 | `P + J` | 暂存区在目标根目录，使用 rename 移到正式目录，物理媒体不重复占用。预检必须确认两者同卷。 |
| SQLite 提交 | **`P + D + J + H`** | WAL、索引页和新缓存 blob 与已经提升的媒体同时存在，这是开始合并必须预留的峰值。 |
...
这样，即使完整预检需要较长时间，也不会在它计算目标目录、缓存映射和容量时让当前库发生变化。
...
| 数据转换 | 每张来源 SQLite 表都在转换注册表中有明确策略；需要保留的业务 JSON 可解析、UUID/路径引用都在映射中；需要保留的二进制缓存可解码或被明确标为可重建缓存。 |
| 文件与缓存对应 | 业务数据引用的每个文件都在计划中；绝对路径不能逃出来源根；复制后应保留的缓存都能对应到一个确定的目标文件与目标 `stat`。 |
...
被明确分类为派生缓存的数据；它不是业务数据丢失，完成页必须如实报告重建数量。
...
- `song_cache.info_json.filePath`、缓存键、指纹并集按映射正确写入。
...
- 来源的节点树、文件树、业务 JSON、缓存映射、目标路径/大小写冲突和媒体可读性必须在预检阶段
...
- 进度显示真实字节和文件数；大文件、很多小文件、缓存较大时仍可理解。
