...
- latest sealed-intake `test` 批次：Rekordbox playlist 当前有效 327 首；音频已在
  `sealed-intake`，dry-run 为复制 0 首、跳过 327 首；truth 和 feature-cache 已生成。
- latest sealed-intake 状态：已被 head near-zero 开发回归消耗，不再是 sealed；音频当前位于
...
当前主 truth selected（`constant-grid-dp` + locked ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2 + head near-zero）：

- pass：702
- fail：229
- pass rate：75.40%
- error：0
- 失败分类：`first-beat-phase` 179，`downbeat` 32，`bpm` 17，`half-or-double-bpm` 1
- guard 计数：`legacy-fallback-low-confidence` 858，`constant-grid-dp-head-near-zero-switch` 5，
...
当前 blind selected（`constant-grid-dp` + locked ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2 + head near-zero）：

- pass：436 / 608 = 71.71%
- error：0
- 失败分类：`first-beat-phase` 127，`downbeat` 29，`bpm` 15，
  `half-or-double-bpm` 1
- candidate oracle：599 / 608 = 98.52%
- 没有 passing candidate：9
- 有 passing candidate 但 scorer 未选中：163
- guard 计数：`legacy-fallback-low-confidence` 568，`constant-grid-dp-head-near-zero-switch` 3，
...
latest test327 selected（已消耗 sealed-intake 批次）：
...
- 分阶段回归口径仍是 current
  `685 -> 694 -> 695 -> 696 -> 701 -> 702`、blind `425 -> 430 -> 430 -> 432 -> 434 -> 435`、old consumed test
  `274 -> 277 -> 278 -> 279`；latest test-new-357 只能记作 `229 -> 230 -> 231`。

失败分类：

- `first-beat-phase`：179
- `downbeat`：32
- `bpm`：17
- `half-or-double-bpm`：1
...
- 有 passing candidate 但 scorer 未选中：197
- 最终选择来源：legacy fallback 874，candidate solver switch 57
...
- `D:/FRKB_database-E/library/FilterLibrary/sample`：current pass 样本，当前目标数量 702。
- `D:/FRKB_database-E/library/FilterLibrary/grid-failures-current`：current 非 pass / error 样本，当前目标数量 229。
...
- `D:/FRKB_database-E/library/FilterLibrary/sealed-intake`：latest test-new-357 已消耗批次当前仍在此处；
...
legacy weakness v3、rank1 structural phase v2 和 head near-zero 都只是在窄 guard 下带来小幅干净提升。`test` 批次已被消耗，
current / blind 也都是已看过数据；下一步不能继续扫阈值或扩大 guard，只能拿 fresh sealed 样本原样复验，
或另找新的结构性 phase evidence。

fresh sealed 的第一目标不是现场涨分，而是验证当前 production solver 是否原样泛化。下一批新样本必须先
锁住 `rank1 structural phase v2` 与 `head near-zero` 现有阈值，记录 pass rate、error、candidate oracle、guard 分布、
...
locked ranker、integer BPM snap、rank1 material legacy weakness v3、rank1 structural phase v2 和 head near-zero；这一步就是主验收输出，
