...
- sealed-eval 状态：已被本轮验收和后续回归优化消耗，不再是 sealed；后续只能作为普通回归集，
  音频长期放在 `D:/FRKB_database-E/library/FilterLibrary/sealed-eval`。
- sealed-eval 音频归档校验：`sync_rekordbox_playlist_audio.py --playlist "test" --target-root D:/FRKB_database-E/library/FilterLibrary/sealed-eval --dry-run`
  显示 `copyCount = 0`。
...
2026-05-09 重新完整跑 `scripts/rkb_phase_ranker_rising_edge_locked_replay.py`：

- 默认 current/blind replay 现在以已集成 locked ranker、integer BPM snap 和 rank1 material
  legacy weakness 的 `frkb-current-latest.json` 为 baseline，因此输出 current `696 -> 697`、
  blind `425 -> 432`，两边 `pass -> fail = 0`。
- 带 sealed/test 参数完整 replay 输出 current `696 -> 697`、blind `425 -> 432`、
  test `274 -> 277`，三套集合 `pass -> fail = 0`。
- 这只是当前 latest 基线上的 sanity check；分阶段回归口径仍是 current
  `685 -> 694 -> 695 -> 696`、blind `425 -> 430 -> 430 -> 432`、test
  `274 -> 277 -> 278 -> 279`。
...
- `D:/FRKB_database-E/library/FilterLibrary/sample`：current pass 样本，当前目标数量 696。
- `D:/FRKB_database-E/library/FilterLibrary/grid-failures-current`：current 非 pass / error 样本，当前目标数量 235。
- `D:/FRKB_database-E/library/FilterLibrary/blind-rekordbox-truth`：blind truth 音频归档，608 首。
- `D:/FRKB_database-E/library/FilterLibrary/sealed-eval`：已消耗 sealed/test 回归音频归档，377 首。
- `D:/FRKB_database-E/library/FilterLibrary/sealed-intake`：下一批 fresh sealed 的固定临时入口，稳定状态必须为空。
