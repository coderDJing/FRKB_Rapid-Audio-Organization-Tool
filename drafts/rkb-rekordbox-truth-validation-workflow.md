...
- latest sealed-intake `test` 批次：Rekordbox playlist 原始 357 首；复制 357 首；跳过 0 首；
  当前 sealed truth 有效 357 首，feature-cache 已生成。
- latest sealed-intake 状态：已被 v3 / structural phase v2 开发回归消耗，不再是 sealed；音频当前位于
...
当前主 truth selected（`constant-grid-dp` + locked ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2）：
...
- guard 计数：`legacy-fallback-low-confidence` 863，`constant-grid-dp-conservative-switch` 29，
...
当前 blind selected（`constant-grid-dp` + locked ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2）：

- pass：435 / 608 = 71.55%
- error：0
- 失败分类：`first-beat-phase` 128，`downbeat` 29，`bpm` 15，
  `half-or-double-bpm` 1
- candidate oracle：599 / 608 = 98.52%
- 没有 passing candidate：9
- 有 passing candidate 但 scorer 未选中：164
- guard 计数：`legacy-fallback-low-confidence` 571，`constant-grid-dp-conservative-switch` 13，
...
- 相比 phase evidence v2 净增 10 首；相比 v3 净增 3 首；逐曲 diff 没有
...
legacy weakness v3 和 rank1 structural phase v2 都只是在窄 guard 下带来小幅干净提升。`test` 批次已被消耗，
...
锁住 `rank1 structural phase v2` 现有阈值，记录 pass rate、error、candidate oracle、guard 分布和
低概率高证据分支触发曲目；rank<=8/topN、降阈值、half/double BPM switch 只能作为离线诊断，不允许在
...
locked ranker、integer BPM snap、rank1 material legacy weakness v3 和 rank1 structural phase v2；这一步就是主验收输出，
