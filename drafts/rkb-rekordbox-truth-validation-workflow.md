...
- 当前验收容差：5ms
- Rekordbox `test` playlist：609 首原始曲目，其中 608 首已归档为 blind truth，1 首因已在主 truth 中跳过
...
- latest sealed-intake `test353` 批次：Rekordbox playlist 当前有效 353 首；音频已复制到
  `sealed-intake`，truth、feature-cache 和 production benchmark 已生成。
- latest sealed-intake 状态：已被 rank1 negative legacy score 开发回归消耗，不再是 sealed；音频当前位于
  `D:/FRKB_database-E/library/FilterLibrary/sealed-intake`。归档/清空前，下一批 fresh sealed
  不得复用这个目录。
- previous sealed-intake `test327` 批次：327 首，已被 head near-zero 开发回归消耗，只作为普通回归素材。
- previous sealed-intake `test-new-357` 批次：357 首，已被 v3 / structural phase v2 开发回归消耗，
  只作为普通回归素材。
- old consumed sealed-eval 归档：上一批 `test` 377 首音频仍长期放在
  `D:/FRKB_database-E/library/FilterLibrary/sealed-eval`，只作为普通回归素材。
- 临时 benchmark progress 文件：不作为长期状态，存在时只视为可复跑的中间产物

当前主 truth selected（`constant-grid-dp` + locked ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2 + head near-zero + rank1 negative legacy score）：

- pass：706
- fail：225
- pass rate：75.83%
- error：0
- 失败分类：`first-beat-phase` 176，`downbeat` 32，`bpm` 16，`half-or-double-bpm` 1
- guard 计数：`legacy-fallback-low-confidence` 852，`constant-grid-dp-rank1-negative-legacy-score-switch` 6，
  `constant-grid-dp-head-near-zero-switch` 5，
  `constant-grid-dp-conservative-switch` 29，
  `constant-grid-dp-locked-rising-edge-ranker` 15，`legacy-fallback-integer-bpm-snap` 11，
  `constant-grid-dp-rank1-structural-phase-switch` 9，
...
当前 blind selected（`constant-grid-dp` + locked ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2 + head near-zero + rank1 negative legacy score）：
...
- 相比 phase evidence v2 净增 11 首；相比 structural phase v2 净增 1 首；逐曲 diff 没有
  `pass -> fail`。rank1 negative legacy score 在 blind 只改变错误类型，不新增 pass。这里仍是已看过
...
- error：0
- 失败分类：`first-beat-phase` 82，`downbeat` 17，`bpm` 17，`half-or-double-bpm` 12，
  `grid-drift` 1
- candidate oracle：338 / 353 = 95.75%
- 没有 passing candidate：15
- 有 passing candidate 但 scorer 未选中：114
...
- rank1 negative legacy score 相比 baseline：`222 -> 224`；current / blind / test353 逐曲 diff 没有
  `pass -> fail`。这批已经被优化消耗，不是新的泛化证明。

consumed test327 selected（已消耗 sealed-intake 批次）：

- pass：215 / 327 = 65.75%
- error：0
- 失败分类：`first-beat-phase` 65，`downbeat` 26，`bpm` 8，
  `half-or-double-bpm` 13
- candidate oracle：316 / 327 = 96.64%
- 没有 passing candidate：11
- 有 passing candidate 但 scorer 未选中：101
- guard 计数：`legacy-fallback-low-confidence` 281，
...
- 有 passing candidate 但 scorer 未选中：193
- 最终选择来源：legacy fallback 863，candidate solver non-legacy source 68

当前 5ms benchmark、classification 和派生视图已经刷新完毕。音频库整理口径如下。
...
- `D:/FRKB_database-E/library/FilterLibrary/sample`：current pass 样本，当前目标数量 706。
- `D:/FRKB_database-E/library/FilterLibrary/grid-failures-current`：current 非 pass / error 样本，当前目标数量 225。
- `D:/FRKB_database-E/library/FilterLibrary/blind-rekordbox-truth`：blind truth 音频归档，608 首。
- `D:/FRKB_database-E/library/FilterLibrary/sealed-eval`：old consumed sealed/test 回归音频归档，377 首。
- `D:/FRKB_database-E/library/FilterLibrary/sealed-intake`：latest test-new-357、test327、test353
  已消耗批次当前仍在此处，共 1037 首；
  归档/清空后才可作为下一批 fresh sealed 的固定临时入口。

`D:/FRKB_database-B` 只作为历史样本库来源保留，不再作为 FRKB-5 脚本默认目标。禁止再新建
`Benchmark/current-*` 或按批次命名的长期 sealed 歌单。当前音频目录按最新 classification
...
legacy weakness v3、rank1 structural phase v2、head near-zero 和 rank1 negative legacy score 都只是在窄
...
锁住 `rank1 structural phase v2`、`head near-zero` 与 `rank1 negative legacy score` 现有阈值，
记录 pass rate、error、candidate oracle、guard 分布、
低概率高证据分支和 head near-zero 触发曲目；rank<=8/topN、降阈值、half/double BPM switch 只能作为离线诊断，不允许在
...
head near-zero 和 rank1 negative legacy score；这一步就是主验收输出，
