...
3. 把剩余音频复制到 `blind-rekordbox-truth/audio/`。
...
下一次新样本验收只走一次性 sealed-eval，不新建长期第三套样本池。用户在 Rekordbox
