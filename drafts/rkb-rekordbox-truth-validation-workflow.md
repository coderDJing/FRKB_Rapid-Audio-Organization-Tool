...
- latest sealed-intake 状态：已被 rank1 negative legacy score v1/v2 开发回归消耗，不再是 sealed；音频当前位于
...
当前主 truth production latest（`constant-grid-dp` + locked ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2 + rank1 negative legacy score v2 + head near-zero + rank1 octave-down）：
...
- 输出：`grid-analysis-lab/rkb-rekordbox-benchmark/frkb-current-latest.json`；
  刷新来源为 `grid-analysis-lab/rkb-rekordbox-benchmark/frkb-current-rank1-octave-down.json`
- 失败分类：`first-beat-phase` 176，`downbeat` 32，`bpm` 16
- guard 计数：`legacy-fallback-low-confidence` 851，`constant-grid-dp-rank1-negative-legacy-score-switch` 6，
  ...
  `constant-grid-dp-phase-evidence-switch` 1，`constant-grid-dp-rank1-octave-down-switch` 1
...
当前 blind selected（`constant-grid-dp` + locked ranker + integer BPM snap + rank1 material legacy weakness v3 + rank1 structural phase v2 + rank1 negative legacy score v2 + head near-zero + rank1 octave-down）：
...
- 相比 phase evidence v2 净增 11 首；相比 structural phase v2 净增 1 首；逐曲 diff 没有
  ...
  在 blind 没有触发。这里仍是已看过
  blind 上的开发回归，不是新的泛化证明。

latest test353 selected（已消耗 sealed-intake 批次）：

- baseline：222 / 353 = 62.89%
- final selected（rank1 negative legacy score v2 + rank1 octave-down）：224 / 353 = 63.46%
- error：0
- 失败分类：`first-beat-phase` 84，`downbeat` 15，`bpm` 17，`half-or-double-bpm` 12，
  `grid-drift` 1
...
- 分阶段回归口径仍是 current
  ...
  latest test327 只能记作 `212 -> 215 -> 218`，latest test353 只能记作 `222 -> 224 -> 224`，
...
- 有 passing candidate 但 scorer 未选中：192
- 最终选择来源：legacy fallback 862，candidate solver non-legacy source 69

当前 5ms benchmark、classification、派生视图和音频目录已经刷新到 `707 / 931 = 75.94%`、
...
- `D:/FRKB_database-E/library/FilterLibrary/sample`：current pass 样本，当前目标数量 707。
...
最新 classification 与音频目录同步后，期望结果：`moveCount = 0`。本轮实际移动
`Umoja - Rabia (Original Mix).mp3` 从 `grid-failures-current` 到 `sample` 后，
dry-run 已归零。
...
legacy weakness v3、rank1 structural phase v2、rank1 negative legacy score v2、head near-zero
和 rank1 octave-down 都只是在窄
...
与 `rank1 octave-down` 现有阈值，
记录 pass rate、error、candidate oracle、guard 分布、
低概率高证据分支、rank1 negative legacy score v2、head near-zero 和 rank1 octave-down
...
rank1 negative legacy score v2、head near-zero 和 rank1 octave-down；这一步就是主验收输出，
