...
- fail：247
- pass rate：73.47%
- 阈值口径变化：`2ms -> 5ms` 后 `fail -> pass` 90，`pass -> fail` 0，净增 90

失败分类：

- `first-beat-phase`：198
- `downbeat`：28
- `bpm`：20
...
- 有 passing candidate 但 scorer 未选中：215
- 最终选择来源：legacy fallback 902，candidate solver conservative switch 29
...
5ms classification 与音频目录同步后，期望结果：`moveCount = 0`。

当前主要问题已经从候选覆盖不足转成 selector / phase 语义不清。当前 candidate solver
候选池能覆盖大多数正确 grid，但现有音频峰值、BeatThis beat 和 attack envelope 特征无法稳定
判断 Rekordbox 风格 first beat 应贴前缘、峰值中心还是视觉网格习惯。下一步不继续堆
selector 权重，也不再维护人工 HTML 观察工具；优先做可复现的离线诊断和 blind truth 验收，
用分布统计确认相位语义。
