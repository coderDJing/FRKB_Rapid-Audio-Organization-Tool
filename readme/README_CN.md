...
**FRKB** 是一款为音频工作者（如 DJ）设计的跨平台桌面音频工作流工具。它把快速整理、指纹去重、波形试听，以及自动录制、Stem 分轨、Pioneer U 盘库支持、全局搜歌这些仍在持续打磨的 Beta 模块，收进同一个 Windows/macOS 桌面应用里。

### Beta 功能亮点

以下模块当前按 Beta 标注，后续仍会继续调整交互和工作流细节。

- **Mixtape 自动录制工作台（Beta）**：独立自动录制窗口与双轨时间线，支持节拍网格对齐、首拍分析、节拍器、增益/BPM 包络、段落静音、撤销，以及和时间线听感一致的导出。
...
- **Mixtape 时间线编辑（Beta）**：独立自动录制工作台、节拍网格编辑、预览同步、参数包络与时间线一致导出。
- **Stem 运行时与分轨（Beta）**：受管运行时、分轨缓存以及多种加速后端，便于为自动录制准备素材。
