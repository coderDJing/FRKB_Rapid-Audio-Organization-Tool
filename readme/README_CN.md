...
**FRKB** 是一款为音频工作者（如 DJ）设计的跨平台桌面音频工作流工具。它把快速整理、指纹去重、波形试听，以及自动混音准备、Stem 分轨、Pioneer U 盘库支持、全局搜歌这些仍在持续打磨的 Beta 模块，收进同一个 Windows/macOS 桌面应用里。

### Beta 功能亮点

以下模块当前按 Beta 标注，后续仍会继续调整交互和工作流细节。

- **Mixtape 自动混音工作台（Beta）**：独立自动混音窗口与双轨时间线，支持节拍网格对齐、首拍分析、节拍器、增益/BPM 包络、段落静音、撤销，以及和时间线听感一致的导出。
- **Stem 分轨链路（Beta）**：支持 stem 模式、分轨缓存、Demucs 运行时管理、ONNX fast 分离、DirectML 调度、XPU 加速，以及按需下载运行时。
- **Pioneer U 盘库工作流（Beta）**：支持旧 Device Library 和 OneLibrary，提供预览波形缓存、多盘识别、U 盘库播放与受限操作。
- **全局搜歌与独立转换（Beta）**：支持全局搜歌和更稳定的定位反馈，同时提供独立格式转换工具，方便在主库之外做批量处理。
- **闲时后台调度（Beta）**：统一闲时任务调度与限流，让后台分析更稳，别老跟前台操作抢资源。
...
- **Mixtape 时间线编辑（Beta）**：独立自动混音工作台、节拍网格编辑、预览同步、参数包络与时间线一致导出。
- **Stem 运行时与分轨（Beta）**：受管运行时、分轨缓存以及多种加速后端，便于为混音准备素材。
- **Pioneer U 盘库支持（Beta）**：支持更多 Pioneer U 盘库形态，包括旧 Device Library 与 OneLibrary 入口分组。
- **全局搜歌（Beta）**：支持跨界面搜歌，并更清晰地回到目标位置。
  | 格式     | 扫描/播放 | 转换 |
  | -------- | --------- | ---- |
  | `MP3`    | ✅         | ✅    |
  | `WAV`    | ✅         | ✅    |
