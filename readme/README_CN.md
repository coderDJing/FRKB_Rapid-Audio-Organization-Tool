...
**FRKB** 是一款面向 DJ 和音乐收藏整理场景的桌面音频工作流工具。它把真实文件整理、波形试听、指纹去重、Rekordbox 与 Pioneer 库读取、双轨横推试听、录音、元数据补齐、格式转换和 Mixtape 编排整合在同一个 Windows/macOS 应用里。
...
- **节拍对齐工具**：支持网格调整、预览同步、节拍器工具，并保持播放与编辑后的网格数据一致。
...
- **相似歌曲查询**：从多个来源检索风格相近的歌曲，辅助扩展选曲范围。
- **云端指纹同步**：支持将本地 SHA256 指纹与 FRKB 云端双向同步，包含差异分析、分页拉取、分批上传、配额和频控。
...
长期目标是引入 AI 模型，学习用户的分类习惯后辅助自动入库。
