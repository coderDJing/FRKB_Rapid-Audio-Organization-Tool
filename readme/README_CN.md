...
**FRKB** 是一款为音频工作者（如 DJ）设计的跨平台桌面音频工作流工具。它把快速整理、指纹去重、波形试听、自动录制、Stem 分轨、Pioneer U 盘库支持、全局搜歌和独立格式转换，收进同一个 Windows/macOS 桌面应用里。
...
- **多格式播放与转换**：内建 FFmpeg 管线，导入即播，支持独立格式转换，批量转码 20+ 常见与专业音频格式。
- **Mixtape 时间线编辑**：独立自动录制工作台与双轨时间线，支持节拍网格编辑、首拍分析、节拍器、预览同步、增益/BPM 包络、段落静音、撤销，以及时间线一致导出。
