...
export const drawStemWaveform = (
  ...
  if (!all) return
  const peakLeft = all.peakLeft || all.left
  const peakRight = all.peakRight || all.right
  ...
  for (let x = 0; x < length; x += 1) {
    ...
    if (useInterpolatedSamples) {
      ...
      const peakLeft0 = peakLeft[i0]
      const peakLeft1 = peakLeft[i1]
      const peakRight0 = peakRight[i0]
      const peakRight1 = peakRight[i1]
      ...
      for (let frameIndex = frameStart; frameIndex <= frameEnd; frameIndex += 1) {
        ...
        const allLeft = peakLeft[frameIndex]
        const allRight = peakRight[frameIndex]
...
export const drawMixxxRgbWaveform = (
  ...
  for (let x = 0; x < length; x += 1) {
    ...
    if (useInterpolatedSamples) {
      ...
      const i1 = Math.min(endFrame - 1, i0 + 1)
      const t = framePos - i0
      const lerp2 = (a: number, b: number) => a + (b - a) * t

      const lowLeft = lerp2(low.left[i0], low.left[i1])
      const lowRight = lerp2(low.right[i0], low.right[i1])
      const midLeft = lerp2(mid.left[i0], mid.left[i1])
      const midRight = lerp2(mid.right[i0], mid.right[i1])
      const highLeft = lerp2(high.left[i0], high.left[i1])
      const highRight = lerp2(high.right[i0], high.right[i1])
      const allAvgLeft = lerp2(all.left[i0], all.left[i1])
      const allAvgRight = lerp2(all.right[i0], all.right[i1])
      ...
      const allLeft = lerp2(peakLeft0, peakLeft1)
      const allRight = lerp2(peakRight0, peakRight1)
