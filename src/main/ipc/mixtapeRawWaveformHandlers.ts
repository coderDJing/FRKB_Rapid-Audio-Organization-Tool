...
const resolveRawWaveformStreamDeckKey = (value: unknown, requestId: string) => {
  const explicit = typeof value === 'string' ? value.trim().toLowerCase() : ''
  if (explicit === 'top' || explicit === 'bottom') return explicit
  const inferred = requestId.includes('-up-') ? 'top' : requestId.includes('-down-') ? 'bottom' : ''
  return inferred || `request:${requestId}`
}
...
export function registerMixtapeRawWaveformHandlers() {
  ...
  const listActiveRawWaveformStreamRequestsForDeck = (deckKey: string) =>
    listActiveRawWaveformStreamRequests().filter((request) => request.deckKey === deckKey)

  const canStartRawWaveformStreamRequest = (request: RawWaveformStreamRequest) =>
    listActiveRawWaveformStreamRequests().length < MAX_ACTIVE_RAW_WAVEFORM_STREAMS &&
    listActiveRawWaveformStreamRequestsForDeck(request.deckKey).length <
  ...
  const getHighestPriorityStartablePendingRawWaveformStreamRequest = () => {
    const pending = listPendingRawWaveformStreamRequests().filter((request) =>
      canStartRawWaveformStreamRequest(request)
    )
    if (pending.length === 0) return null
    pending.sort((left, right) =>
      isHigherPriorityRawWaveformStreamRequest(left, right)
  ...
  const getLowestPriorityActiveRawWaveformStreamRequestForDeck = (deckKey: string) => {
    const active = listActiveRawWaveformStreamRequestsForDeck(deckKey)
    if (active.length === 0) return null
    active.sort((left, right) =>
      isHigherPriorityRawWaveformStreamRequest(left, right)
        ? -1
  ...
  const drainRawWaveformStreamQueue = () => {
    while (listActiveRawWaveformStreamRequests().length < MAX_ACTIVE_RAW_WAVEFORM_STREAMS) {
      const nextRequest = getHighestPriorityStartablePendingRawWaveformStreamRequest()
  ...
  const rebalanceRawWaveformStreamQueue = () => {
    ...
    const sameDeckActive = highestPending
      ? getLowestPriorityActiveRawWaveformStreamRequestForDeck(highestPending.deckKey)
      : null
    if (
      highestPending &&
      sameDeckActive &&
      isHigherPriorityRawWaveformStreamRequest(highestPending, sameDeckActive)
    ) {
      clearRawWaveformStreamRequest(sameDeckActive.requestId, { keepQueued: true })
    } else if (
      highestPending &&
      listActiveRawWaveformStreamRequests().length >= MAX_ACTIVE_RAW_WAVEFORM_STREAMS
    ) {
  ...
  ipcMain.on(
    ...
    (
      _event,
      payload: { requestId?: string; priorityHint?: number; protectsPlayback?: boolean }
    ) => {
      const requestId = String(payload?.requestId || '').trim()
      if (!requestId) return
      const request = rawWaveformStreamRequests.get(requestId)
