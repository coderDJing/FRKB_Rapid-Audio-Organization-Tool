...
import { searchMusicBrainz, fetchMusicBrainzSuggestion } from './musicBrainz'
...
function createProgressEmitter(progressId: string): ProgressEmitter {
...
export async function autoFillTrackMetadata(
  ...
  const pushProgress = createProgressEmitter(progressId)
  ...
  for (let idx = 0; idx < uniquePaths.length; idx++) {
    ...
    try {
      ...
      const updatePayload = suggestion ? buildUpdatePayload(filePath, suggestion.suggestion) : null
