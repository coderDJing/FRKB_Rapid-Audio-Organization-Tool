...
export const getLibraryMergeBusyReasons = (
  options: { includeBackgroundTask?: boolean } = {}
): string[] => {
  ...
  if (options.includeBackgroundTask !== false) {
...
export const acquireLibraryMergeMutationLock = async (
  ...
  const reasons = getLibraryMergeBusyReasons({ includeBackgroundTask: false })
  if (reasons.length > 0) {
