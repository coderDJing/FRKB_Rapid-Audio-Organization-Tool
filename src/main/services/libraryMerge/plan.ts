...
const getCapacity = async (
  ...
  const beforeSafety = payloadBytes + databasePeakBytes + journalBytes
  const safetyBytes = Math.max(2 * 1024 * 1024 * 1024, Math.ceil(beforeSafety * 0.15))
  const requiredBytes = beforeSafety + safetyBytes
  const availableBytes = await getAvailableBytes(targetRoot)
...
export async function buildLibraryMergePlan(params: {
  ...
  if (params.appVersion && !isManifestCompatible(sourceManifest, params.appVersion)) {
    throw new LibraryMergeError('SOURCE_VERSION_INCOMPATIBLE', '来源库需要更高版本的 FRKB')
  }
  ...
  if (getUserVersion(params.sourceDb) !== getUserVersion(params.targetDb)) {
    throw new LibraryMergeError('SOURCE_SCHEMA_UNSUPPORTED', '来源库与当前库的数据库版本不一致')
  }
