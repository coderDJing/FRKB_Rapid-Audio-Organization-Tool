...
type LibraryNodeType = 'root' | 'library' | 'dir' | 'songList'
...
function normalizeOrder(value: any): number | null {
...
async function readUuidMarker(dirPath: string): Promise<string | null> {
...
async function writeUuidMarker(dirPath: string, uuid: string): Promise<void> {
...
function getPathDepth(relPath: string): number {
  return splitPath(relPath).length
}

function isNodeType(value: any): value is LibraryNodeType {
  return value === 'root' || value === 'library' || value === 'dir' || value === 'songList'
}

function isChildNodeType(value: any): value is Exclude<LibraryNodeType, 'root'> {
  return value === 'library' || value === 'dir' || value === 'songList'
}

function toNodeRow(row: any): LibraryNodeRow | null {
...
function getDb(dbRoot?: string): any | null {
  if (dbRoot) return initLibraryDb(dbRoot)
  return getLibraryDb()
}

function getRootNode(db: any): LibraryNodeRow | null {
  try {
    const row =
      db
        .prepare(
...
function ensureRootNode(db: any): LibraryNodeRow | null {
...
export async function ensureLibraryTreeBaseline(
  dbRoot: string,
  options: {
    coreDirNames?: { FilterLibrary: string; CuratedLibrary: string; RecycleBin: string }
  } = {}
): Promise<void> {
  if (!dbRoot) return
  ...
  try {
    ...
    const run = db.transaction(() => {
      ...
      insert.run(recycleUuid, root.uuid, coreNames.RecycleBin, 'library', 3)
    })
    run()
...
export async function syncLibraryTreeFromDisk(
  ...
  const coreOrderMap = new Map<string, number>([
    ...
    [coreNames.RecycleBin, 3]
  ])
  const coreNameList = Array.from(
    new Set([coreNames.FilterLibrary, coreNames.CuratedLibrary, coreNames.RecycleBin])
