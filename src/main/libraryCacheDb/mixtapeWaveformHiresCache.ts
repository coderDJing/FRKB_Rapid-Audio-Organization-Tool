...
const MIXTAPE_HIRES_WAVEFORM_TABLE = 'mixtape_waveform_hires_cache'

const normalizeTargetRate = (value: unknown, fallback = 4000) => {
...
function normalizeWaveformMeta(row: any): WaveformCacheMeta | null {
  ...
  if (targetRate <= 0 || frames <= 0) return null
  return { size, mtimeMs, version, targetRate, sampleRate, step, duration, frames }
}

export async function loadMixtapeWaveformHiresCacheData(
  listRoot: string,
  filePath: string,
  targetRate: number,
  ...
  try {
    let row = db
      .prepare(
        `SELECT size, mtime_ms, version, target_rate, sample_rate, step, duration, frames, data FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ? AND file_path = ? AND target_rate = ?`
      )
      .get(listRootKey, fileKey, rate)
    ...
    if (!row && fileKeyRaw) {
      row = db
        ...
        .get(listRootKey, fileKeyRaw, rate)
      if (row) {
    ...
    if (!row && legacyListRoot && legacyFilePath) {
      row = db
        ...
        .get(legacyListRoot, legacyFilePath, rate)
      if (row) {
    ...
    if (!meta || meta.version !== MIXXX_MIXTAPE_WAVEFORM_CACHE_VERSION) {
      await removeMixtapeWaveformHiresCacheEntry(listRoot, filePath, rate)
      return null
    }
      ...
      try {
        ...
        if (sameRoot) {
          const del = db.prepare(
            `DELETE FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ? AND file_path = ? AND target_rate = ?`
          )
          const update = db.prepare(
            `UPDATE ${MIXTAPE_HIRES_WAVEFORM_TABLE} SET list_root = ?, file_path = ? WHERE list_root = ? AND file_path = ? AND target_rate = ?`
          )
          del.run(listRootKey, fileKey, rate)
          update.run(listRootKey, fileKey, hitListRoot, hitFilePath, rate)
        } else {
          await upsertMixtapeWaveformHiresCacheEntry(listRoot, filePath, rate, stat, decoded)
        }
      } catch {}
    }
    return decoded
  } catch (error) {
    log.error('[sqlite] mixtape waveform hires cache load failed', error)
    return undefined
  }
}

export async function upsertMixtapeWaveformHiresCacheEntry(
  listRoot: string,
  filePath: string,
  targetRate: number,
  stat: { size: number; mtimeMs: number },
  ...
  try {
    ...
    if (resolvedFile.keyRaw && resolvedFile.keyRaw !== resolvedFile.key) {
      db.prepare(
        `DELETE FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ? AND file_path = ? AND target_rate = ?`
      ).run(listRootKey, resolvedFile.keyRaw, rate)
    }
    if (legacyListRoot && resolvedFile.legacyAbs) {
      db.prepare(
        `DELETE FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ? AND file_path = ? AND target_rate = ?`
      ).run(legacyListRoot, resolvedFile.legacyAbs, rate)
    }
    return true
  } catch (error) {
    log.error('[sqlite] mixtape waveform hires cache upsert failed', error)
    return false
  }
}

export async function removeMixtapeWaveformHiresCacheEntry(
  listRoot: string,
  filePath: string,
  targetRate?: number
): Promise<boolean> {
  ...
  const legacyListRoot =
    resolvedRoot.legacyAbs && resolvedRoot.legacyAbs !== listRootKey
      ? resolvedRoot.legacyAbs
      : undefined
  const hasTargetRate = Number.isFinite(targetRate) && Number(targetRate) > 0
  const rate = hasTargetRate ? normalizeTargetRate(targetRate) : 0
  try {
    if (hasTargetRate) {
      const del = db.prepare(
        `DELETE FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ? AND file_path = ? AND target_rate = ?`
      )
      del.run(listRootKey, resolvedFile.key, rate)
      if (resolvedFile.keyRaw) del.run(listRootKey, resolvedFile.keyRaw, rate)
      if (legacyListRoot && resolvedFile.legacyAbs) {
        del.run(legacyListRoot, resolvedFile.legacyAbs, rate)
      }
      return true
    }

    const del = db.prepare(
      `DELETE FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ? AND file_path = ?`
    )
    ...
    log.error('[sqlite] mixtape waveform hires cache delete failed', error)
    return false
  }
}

export async function clearMixtapeWaveformHiresCache(
  listRoot: string,
  targetRate?: number
): Promise<boolean> {
  const db = getLibraryDb()
  ...
  try {
    if (hasTargetRate) {
      const del = db.prepare(
        `DELETE FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ? AND target_rate = ?`
      )
      del.run(listRootKey, rate)
      if (legacyListRoot) {
        del.run(legacyListRoot, rate)
      }
      return true
    }

    db.prepare(`DELETE FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ?`).run(listRootKey)
    if (legacyListRoot) {
      db.prepare(`DELETE FROM ${MIXTAPE_HIRES_WAVEFORM_TABLE} WHERE list_root = ?`).run(
        legacyListRoot
      )
    }
    return true
  } catch (error) {
    log.error('[sqlite] mixtape waveform hires cache clear failed', error)
    return false
  }
}
